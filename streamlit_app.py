...
if st.button("✍️ Generate Prompt"):
    if not job_description or not user_cv:
        ...
        prompt = f"""
You are a professional resume writer. Use the job description and the candidate’s background below to rewrite the CV so it aligns with the job, sounds professional,doesn't just match everything in the job description but uses actual things that support the description and feels natural—not AI-generated.
